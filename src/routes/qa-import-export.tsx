...
function QaImportExportPage() {
  ...
  useEffect(() => {
    ...
    // W-X. Build 3.0A — Recipe CSV Import Accepted
    next.push({ label: "W. Recipe CSV Import accepted", status: "pass", detail: "Build 3.0A — live-verified. Recipes + recipe-lines CSV; preview/apply with duplicate + line modes; see /qa-recipe-import" });
    ...
    next.push({ label: "Y. XLS/XLSM import remains future", status: "warn", detail: "Only CSV supported; no formula/macro migration" });
  ...
  return (
    <AppShell>
      <PageHeader title="QA — Import/Export" description="Build 3.0A — Accepted. CSV import (ingredients + recipes), export sanitization, formula injection protection." />
      <div className="space-y-6 p-6">
        ...
        <p className="text-[11px] text-muted-foreground">Build 3.0A — CSV Import/Export Accepted. See /qa-recipe-import for Recipe import details.</p>
      </div>
    </AppShell>
  );
}
