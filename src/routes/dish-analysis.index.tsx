...
export const Route = createFileRoute("/dish-analysis/")({
  ...
  component: DishAnalysisPage,
});

function DishAnalysisPage() {
  const dishes = recipes.filter((r) => r.type === "Dish");
  const [selectedId, setSelectedId] = useState(dishes[0]?.id ?? "");
  const [scenarioCostPct, setScenarioCostPct] = useState([0]); // % cost change
  const [scenarioPricePct, setScenarioPricePct] = useState([0]); // % menu price change
  ...
  return (
    <AppShell>
      ...
      <div className="space-y-6 p-6">
        ...
        {/* Two-column: COGS breakdown + Snapshot delta */}
                  ...
                  {lineRows
                    ...
                    .map(({ line, ingredient, lineCost }) => (
                      <TableRow key={line.id}>
                        ...
                        <TableCell className="text-right">
                          <PercentCell value={lineCost / totalCogs} decimals={1} />
                        </TableCell>
                      </TableRow>
                    ))}
                </TableBody>
            ...
            <CardContent>
              ...
              <ul className="mt-3 space-y-2 text-sm">
                ...
                {lineRows.filter((r) => r.ingredient?.spike).length === 0 && (
                  <li className="text-muted-foreground">No ingredient spikes affecting this dish.</li>
                )}
              </ul>
            </CardContent>
          </Card>
